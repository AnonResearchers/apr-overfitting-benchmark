...
def convert_line_endings_to_unix(file_path):
    # Ensure file uses Unix line endings
...
def normalize_java_files(root_dir):
    # Recursively convert all Java files in a directory to Unix line endings
...
def run_with_timeout(cmd, cwd, timeout):
    # Run a subprocess with a timeout, returning the result or a timeout code
    try:
...
def find_classpath_dirs(base_dir):
    # Find likely class and test output directories for classpath construction
    class_dirs = []
    patterns = [
        "**/build",                    
        "**/build-tests",              
...
def find_test_class_path(test_class, project_dir):
    # Find the file path for a given test class in the project directory
...
def extract_class_body_elements(content, original_class, new_class):
    # Parse Java class and extract methods, fields, constructors, and inner classes
    method_map = {}
...
def extract_test_methods(test_class, class_path):
    # Extract test and helper methods from a Java test class file
    try:
...
def create_test_class(class_info, methods, suffix, project_dir):
    # Generate a new Java test class file with selected methods and a suffix
...
# Set Java 11 for this script's subprocesses
...
patch_info_dir = "repairllama_data/raw_data/patch_info"
patch_meta_dir = os.path.join(patch_info_dir, "repairllama_patches")
...
# Map patch correctness to their directories
patch_dirs = {
...
# Index all patch files by project and version
...
# Process each buggy project-version directory
for project in os.listdir(os.path.join(project_root, "buggy")):
    for version_id in os.listdir(os.path.join(project_root, "buggy", project)):
        ...
        # Copy buggy and fixed project directories to temp
        ...
        # Compile and run Daikon on both buggy and fixed copies
        ...
        for patch_info in project_patches:
            ...
            patch_result = subprocess.run(
                ["patch", "-p0", "--ignore-whitespace", "-i", patch_file_path, "-t"],
                cwd=patched_copy,
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                universal_newlines=True
...
# Write mapping from patch number to filename
