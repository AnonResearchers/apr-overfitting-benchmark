...
# Set up paths for patch, buggy project, and results directories
current_path = os.getcwd()
PATCH_DIRECTORY = "../../datasets/repairllama/context_size_3_filtered/"
BUGGY_PROJECT_DIRECTORY = "../../defects4j_projects/repairllama/buggy"
RESULTS_DIRECTORY = f"{current_path}/patches/patches_entropy_repairllama_filtered"

# Create results directory if it doesn't exist
if not os.path.exists(RESULTS_DIRECTORY):
    os.mkdir(RESULTS_DIRECTORY)

# Initialize the infiller model
...
# Function to form a generation prompt for the infiller model
def form_gen_prompt(code_before, code_after):
    prompt = code_before + "<|mask:0|>" + "\n" + code_after + "<|mask:1|><|mask:0|>"
    return prompt

# Function to extract the first line from generated IDs
...
# Function to form an entropy prompt for the infiller model
...
# Function to calculate entropy for a specific line
...
# Function to checkout a buggy file from the Defects4J dataset
...
# Function to calculate entropy for patched and original files
def defects4j(file_path, project, bug_id, diff_line_dict):
    ...
    for diff_line, (line_change, change_type) in diff_line_dict.items():
        ...
        try:
            # Apply changes to the patched file based on the diff
        ...
        # Extract context before and after the diff line
        ...
        # Generate entropy prompt and calculate entropy
        ...
        # Store entropy values in dictionaries
        if original_entropy:
...
if __name__ == "__main__": 
    ...
    # Iterate over patch categories (correct and overfitting)
    for category in ["correct", "overfitting"]:
        ...
        for subdir, _, files in os.walk(category_path):
            for file in files:
                # Extract project and bug information from the file name
                ...
                for i, patch_line in enumerate(patch_data):
                    if patch_line.startswith("---"):
                        # Extract the Java file path from the patch
                        java_path = patch_line.replace("--- a/", "").replace("\n", "").replace("--- ", "")
                    elif patch_line.startswith("@@"):
                        try:
                            # Extract the initial line number from the patch
                            initial_line = int(
                        ...
                        for j in range(i + 1, len(patch_data) - 1):
                            ...
                            if patch_data_line.startswith("@@"):
                                break
                            # Parse patch lines and classify changes
                ...
                # Calculate entropy for the patch
                ...
                # Update permissions for the results directory
