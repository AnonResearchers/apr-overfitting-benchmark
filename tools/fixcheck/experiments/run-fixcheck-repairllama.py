...
for index, row in df.iterrows():
    ...
    patch_file_location = f'{BASE_PATCH_DIR}/{classification}/{patch_name}'
    buggy_project_location = f'{BASE_BUG_DIR}/buggy/{base_dir}'
    ...
    try:
        result = subprocess.run(
            ["patch", "-p0", "--ignore-whitespace", "-i", patch_file_location, "-t"], # edited for new patches
            cwd=patch_dest_dir,
            capture_output=True,
            text=True,
            check=True
...
# Write time taken to a file
with open('fixcheck_repairllama_time.txt', 'w') as f:
