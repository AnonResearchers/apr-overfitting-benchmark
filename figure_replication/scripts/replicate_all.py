...
def main():
    ...
    parser.add_argument(
        "--figures",
        action="store_true",
        help="If set, skips the RQ1 and RQ3 data processing steps and only generates figures."
    )
    ...
    if not args.figures:
        ...
        print("### RQ1 Data Processing Completed ###")
        print("\n\n### RQ3 Data Processing ###")
        ...
        result = subprocess.run(
            [
                ...
                "--confidence", "85",
                "--format", "csv",
                "--output", f"{output_dir}/tables/rq3/rs85.csv"
            ],
            capture_output=True,
            text=True
        )
        ...
        result = subprocess.run(
            [
                ...
                "--confidence", "95",
                "--format", "csv",
                "--output", f"{output_dir}/tables/rq3/rs95.csv"
            ],
            capture_output=True,
            text=True
        )
        ...
        result = subprocess.run(
            [
                ...
                f"{output_dir}/tables/rq3/rs85.csv",
                f"{output_dir}/tables/rq3/rs95.csv",
                f"{output_dir}/tables/rq3/rs-combined.csv"
            ],
            capture_output=True,
            text=True
        )
    ...
    result = subprocess.run(
        [
            ...
            "--output_dir", f"{output_dir}/visualisations/rq1"
        ],
        capture_output=True,
        text=True
    )
    ...
    result = subprocess.run(
        [
            ...
            "--output",
            f"{output_dir}/visualisations/rq1/apr_tool_mcc_heatmap.png"
        ],
        capture_output=True,
        text=True
    )
    ...
    result = subprocess.run(
        [
            ...
            "--confidence", "85",
            "--format", "latex",
            "--output", f"{output_dir}/tables/rq3/rs85-latex.txt"
        ],
        capture_output=True,
        text=True
    )
    ...
    result = subprocess.run(
        [
            ...
            "--confidence", "95",
            "--format", "latex",
            "--output", f"{output_dir}/tables/rq3/rs95-latex.txt"
        ],
        capture_output=True,
        text=True
    )
    ...
    result = subprocess.run(
        [
            ...
            f"{output_dir}/tables/rq3/rs-combined.csv",
            f"--output={output_dir}/tables/rq3/rs-combined.tex"
        ],
        capture_output=True,
        text=True
    )
    ...
    result = subprocess.run(
        [
            ...
            "--out",
            f"{output_dir}/visualisations/rq3/tools_vs_wbc.png"
        ],
        capture_output=True,
        text=True
    )
